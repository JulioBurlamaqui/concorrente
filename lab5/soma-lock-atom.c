...
void *ExecutaTarefa (void *arg) {
  ...
  while (multiplos < PRIMEIROS) 
  {
    //--entrada na SC
    pthread_mutex_lock(&mutex);
    //--SC (seção critica)
    //printf("\nSoma = %d\n", soma);
    while(!(soma%10)) //entra no laço se 'soma' for multiplo de 10
      pthread_cond_wait(&cond, &mutex); //incrementa a variavel compartilhada
    soma++;
...
void *extra (void *args) {
  ...
  while(multiplos < PRIMEIROS) 
  {
    pthread_mutex_lock(&mutex);
    if(!(soma%10))
    {//entra no laço se 'soma' for multiplo de 10
      printf("multiplo = %ld \n", soma);
      multiplos++;
      pthread_cond_broadcast(&cond);
      soma++;
